...
Dark themes use `default.png`; Light uses `argvus-light.png`; Slate uses `argvus-dark-aether-slate.png`; and Dark Silver uses `argvus-dark-silver.png`. Normal and Float variants in each family share the same wallpaper. Wallpapers ship with the `argvus-appearance` package under `/usr/share/backgrounds/argvus/`.
