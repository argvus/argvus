...
`argvus-storage` is packaged separately. It installs `/usr/bin/argvus-storage`, default configuration under `/etc/argvus-storage/`, and its license under `/usr/share/licenses/argvus-storage/`. See the [Removable Storage](/docs/storage/) page for usage and configuration.

`argvus-calendar` is packaged separately. It installs `/usr/bin/argvus-calendar`, defaults under `/etc/argvus-calendar/` and the user systemd unit at `/usr/lib/systemd/user/argvus-calendar.service`. See the [Calendar](/docs/calendar/) page for usage and configuration.
