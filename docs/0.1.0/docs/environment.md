...
| Notifications and power menu | `config/dunst/`, `config/hypr/scripts/power-menu.sh` | Notification styling and the Rofi-based power/logout menu. |
...
`argvus-storage` and `argvus-calendar` are packaged separately and install their own binaries and system defaults. See [Official Apps](/0.1.0/docs/official-apps/), [Calendar](/0.1.0/docs/calendar/) and [Removable Storage](/0.1.0/docs/storage/).
